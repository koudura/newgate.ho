...
if (isset($_POST['submit'])) {
    $conn = connect();
    $id = Input::post('id');
    $quest = Questionnaire::getByID($conn,$id);
    $count = intval(Input::post('count'));
    $jArray = array();
    for ($i = 1; $i <= $count; $i++) {
        $jArray[$i] = Input::post("ans".$i);
    }
    $jArray = json_encode($jArray);
    $quest->fillByUser($conn, $current_user->id, $jArray);
    unset($_SESSION['qid']);
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                ...
                <div id="questdiv">
                    ...
                    foreach ($currentQ->jsonQ as $key => $value) {
                        $ind = $key;
                        echo '<div> Q'.$key.':<input class="inputext" type="text" name="' . $key . '" value="' . htmlspecialchars($value) . '" required disabled>  </div>';
                        echo '<div>A'.$key.': <input class="inputext" type="text" name="ans' . $key . '" value="" required >  </div>';
                    }
                    ?>
                </div>
                <div class="lobot">
